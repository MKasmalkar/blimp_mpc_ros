...
class PIDLineFollower(BlimpController):
    ...
    def __init__(self, dT, skip_derivatives=True):
        ...
        self.x_pid = PID(1, 0.1, 0, dT)
        self.y_pid = PID(1, 0.1, 0, dT)
        self.z_pid = PID(1, 0.1, 0, dT)
        self.psi_pid = PID(1, 0.1, 0, dT)
    ...
    def get_ctrl_action(self, sim):
        ...
        x_error = self.traj_x[n] - x
        y_error = self.traj_y[n] - y
        z_error = self.traj_z[n] - z
        psi_error = self.traj_psi[n] - psi
        
        f_x = self.x_pid.get_ctrl(x_error)
        f_y = self.y_pid.get_ctrl(y_error)
        f_z = self.z_pid.get_ctrl(z_error)
        tau_z = self.psi_pid.get_ctrl(psi_error)
        
        k_x = np.array([f_x, f_y, f_z, tau_z]).reshape((4, 1))
